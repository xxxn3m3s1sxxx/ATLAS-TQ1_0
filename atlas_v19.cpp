// Atlas v19 - BitNet b1.58 2B4T: ReLU2, RoPE 0=500k, sub_norms, 30 layers
// clang++ -O3 -mavx2 -fopenmp atlas_v19.cpp -o atlas_v19.exe
...
static void mv_tq10_hybrid_omp(const TQ1Mat* m, const float* x, float* y) {
    int rows = m->rows, cols = m->cols, pc = m->packed_cols;
    const float* scales = m->scales;
    const float sign_map[4] = {0.0f, 1.0f, -1.0f, 0.0f};
    #pragma omp for schedule(static)
    for (int r = 0; r < rows; r++) {
        double sum = 0.0;
        ...
        for (int i = 0; i < cols; i += 4) {
            uint8_t byte = rd[i / 4];
            if (byte) {
                sum += sign_map[(byte >> 0) & 3] * (double)x[i];
                sum += sign_map[(byte >> 2) & 3] * (double)x[i+1];
                sum += sign_map[(byte >> 4) & 3] * (double)x[i+2];
                sum += sign_map[(byte >> 6) & 3] * (double)x[i+3];
            }
        }
        int gi = r / m->group_size;
        if (gi >= m->num_groups) gi = m->num_groups - 1;
        y[r] = (float)(sum * scales[gi]);
    }
}
...
static float quantize(const float* s, int8_t* d, int n) {
    ...
    for (int i = 0; i < n; i++) buf[i] = fabs((double)s[i]);
    qsort(buf, n, sizeof(double), cmp_dbl_desc);
    int k = (int)(n * 0.9995f);
    if (k >= n) k = n - 1; if (k < 0) k = 0;
    double ma = fmax(buf[k], 1e-10);
    free(buf);
    double inv_q = 127.0 / ma;
    for (int i = 0; i < n; i++) {
        double v = round((double)s[i] * inv_q);
        if (v > 127.0) v = 127.0; else if (v < -128.0) v = -128.0;
        d[i] = (int8_t)v;
    }
    return (float)(ma / 127.0);
}
...
static void forward_layer(LayerW* l, float* x,
    ...
    #pragma omp parallel
    {
        ...
        #pragma omp single
        {
            for (int i = 0; i < INTERMEDIATE; i++) gate[i] = relu2_f(gate[i]) * up[i];
            rms_norm(gate, l->ffn_sub, INTERMEDIATE);
        }

        mv_tq10_hybrid_omp(&l->td, gate, q);
...
int main() {
    ...
    while (1) {
...
#ifdef DEBUG_LAYERS
                if (pos == 0 && li < 5)
                    fprintf(stderr, "L%d_X: %.10f %.10f %.10f %.10f %.10f\n",
        ...
        {
            // Helper to dump top-5 to stderr
