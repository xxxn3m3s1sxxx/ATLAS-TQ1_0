...
- **f32 bypass**: Auto-enabled for `hidden <= 2048` (1B model). Eliminates activation quantization noise.
...
All: `head_dim=256`, `rope_theta=1000042`, GQA.

## Build

**Windows (clang):**
```bash
clang++ -fopenmp -O2 -mavx2 -mfma -mf16c -ffast-math -std=c++17 -shared -o atlas.dll atlas_api.cpp -lomp
```
