// Atlas v1.7 - Full 28-layer Falcon3 with per-row group scales + stdin pipe
// clang++ -O3 -mavx2 -lm atlas_v17.cpp -o atlas_v17.exe
// New format: each .tern.bin has header: rows, cols, group_size, num_groups, scales[], data[]
...
static TernMat load_tern_grouped(const char* name) {
    char p[512]; snprintf(p, sizeof(p), "%s%s_tern.bin", WDIR, name);
    FILE* f = fopen(p, "rb"); if (!f) { fprintf(stderr, "ERROR: %s\n", p); exit(1); }
    TernMat m;
    fread(&m.rows, 4, 1, f);
    fread(&m.cols, 4, 1, f);
    ...
    m.data = (int8_t*)_aligned_malloc(m.rows * m.cols, 32);
    fread(m.data, 1, m.rows * m.cols, f);
    fclose(f);
    return m;
}

static void mv_tern_scales(const TernMat* m, const int8_t* a, float* y) {
    int rows = m->rows, cols = m->cols;
    const float* scales = m->scales;
    for (int r = 0; r < rows; r++) {
        __m256i s16 = _mm256_setzero_si256();
        for (int i = 0; i < cols; i += 32) {
            __m256i wv = _mm256_loadu_si256((__m256i*)&m->data[r*cols+i]);
        ...
        y[r] = dot * scales[gi];  /* qs applied in caller */
    }
}
...
static void forward_layer(LayerW* l, float* x,
    ...
    mv_tern_scales(&l->tq, ai8, q);  for (int i=0;i<NH_HD;i++) q[i] *= qq;
    mv_tern_scales(&l->tk, ai8, kv); for (int i=0;i<NKV_HD;i++) kv[i] *= qq;
    mv_tern_scales(&l->tv, ai8, kv+NKV_HD); for (int i=0;i<NKV_HD;i++) kv[NKV_HD+i] *= qq;
    ...
    mv_tern_scales(&l->to, ai8, kv); for (int i=0;i<HIDDEN;i++) kv[i] *= qo;
    for (int i = 0; i < HIDDEN; i++) x[i] = res[i] + kv[i];

    memcpy(res, x, HIDDEN*4); rms_norm(x, l->ln2, HIDDEN);
    float qg = quantize(x, ai8, HIDDEN);
    mv_tern_scales(&l->tg, ai8, gate); for (int i=0;i<INTERMEDIATE;i++) gate[i] *= qg;
    mv_tern_scales(&l->tu, ai8, up);   for (int i=0;i<INTERMEDIATE;i++) up[i] *= qg;
    for (int i = 0; i < INTERMEDIATE; i++) gate[i] = silu_f(gate[i]) * up[i];
    float qd = quantize(gate, ai8, INTERMEDIATE);
    mv_tern_scales(&l->td, ai8, q); for (int i=0;i<HIDDEN;i++) q[i] *= qd;
...
int main() {
    ...
    clock_t t0 = clock();
    fprintf(stderr, "Atlas v1.7 - Loading weights (per-row scales)...\n");
